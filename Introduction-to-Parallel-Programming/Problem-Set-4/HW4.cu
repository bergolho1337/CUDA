...
// The cross-correlation is type of filter that uses a template image in order to check how close a part of the original image 
// (this part is the same size of the template).
// If the image part is almost the template than the value of 'sum_of_squared_image_diffs' will be too small. This means that the result of correlation
// will high, since the relation between 'sum_of_squared_image_diffs' and 'result_value' is inversely proportional.
    ...
    )
{
  ...
  if ( image_index_2d.x < nx && image_index_2d.y < ny )
  {
    ...
    for ( int y = -template_half_height; y <= template_half_height; y++ )
    {
      ...
      {
        // Stencil -- The clamped is used to avoid taking out of range pixels from the image
    ...
    for ( int y = -template_half_height; y <= template_half_height; y++ )
    {
      for ( int x = -template_half_width; x <= template_half_width; x++ )
      {
        ...
        unsigned char image_offset_value = d_original[ image_offset_index_1d_clamped ];
        // For each pixel of the image calculate the difference with the image_mean
        ...
        // For each pixel of the template_image calculate the difference with the template_mean
        float         template_diff  = template_value - template_mean;

        // Compute the values used for the cross-relation
...
void preProcess(unsigned int **inputVals,
  ...
  // Split the image
  // We pass iterators to the beginning and ending of the image and a function that will be executed for each pixel of the image
  // This function is implemented in way that for each pixel we will split the 4 channel (R,G,B,A) of the original pixel and store the
  // specific channels on there variables. (The Trust library is implemented using CUDA C, so its operations are parallel on the GPU)
  ...
  // Need to compute the mean for each template channel (The third argument is the initial value of the returned value)
  // The default reduce is the SUM
  ...
  // The returned value is an iterator reference to the minimum and the maximum value of the vector
  ...
  // For each element of the vector we sum '-bias' (which is the inverse of the minimum value)
  // By doing that we will get a positive vector
