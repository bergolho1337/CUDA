...
void loadImageRGBA(const std::string &filename,
                   ...
                   size_t *numRows, size_t *numCols)
{
  // The default image on OpenCV is open in a BGR format (the RGB bytes are reversed)
  ...
  *imagePtr = new uchar4[image.rows * image.cols];
  ...
  // Get a reference to the first byte of the image
  unsigned char *cvPtr = imageRGBA.ptr<unsigned char>(0);
  // Copy the image to adress pointed by 'imagePtr'
